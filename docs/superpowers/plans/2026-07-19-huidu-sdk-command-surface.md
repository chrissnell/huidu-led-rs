...
- **Spec coverage** (`DESIGN.md §12` item 5): `get_device_info` (T1), `set_brightness` manual/scheduled/sensor + get (T2), network Ethernet static/DHCP + Wi-Fi AP/Station (T3), time sync set-time/timezone/DST + get, plus switch-time as the other non-screen scheduling command (T4), boot logo get/set/clear (T5), TCP server config + file listing/delete (T6). Each wrapper builds the request via a Subsystem-2 XML type, `send`s, and parses a typed reply.
- **Test coverage:** every wrapper has a Tier-2 `MockDevice` command-flow test — getters assert a scripted reply parses, setters capture and assert the sent request body. Two additional tests assert a device-reported non-success `result` propagates as `Error::Proto(ProtoError::SdkError)` from both a getter and a setter (the shared `get_body`/`exec` error path).
